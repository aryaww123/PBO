...
class Client:
    def __init__(self, host, port):
        self.HOST = host
        self.PORT = port
        self.username = simpledialog.askstring("Nickname", "Please choose a nickname")
    ...
    def add_user(self):
        if self.username:
            self.text_area.config(state='normal')
            self.text_area.insert('end', f"User '{self.username}' added.\n")
            self.text_area.yview('end')
            self.text_area.config(state='disabled')

            self.users.add(self.username)
            self.user_messages.setdefault(self.username, [])

            self.sock.send(f"ADD_USER {self.username}".encode('utf-8'))

            # Use subprocess.Popen to open a new window for the new user
            subprocess.Popen(['python', 'client-trial.py', self.username])
        else:
